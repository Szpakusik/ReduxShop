...
const Header = props => {
  ...
  useEffect(() => {
    if (localStorage.getItem('JWT')) props.signIn();
    ...
    <header className={`header ${headerStyles.header}`}>
      <nav className="navbar navbar-light navbar-expand-lg navbar-fixed-top sticky-header p-0">
        <div className="container-fluid p-0 overflow-hidden d-none d-md-flex">
          <Navbar.Brand
            className={`banner navbar-header col-xs-12 col-sm-4 ${headerStyles.banner}`}
          >
            <a
              className="navbar-brand text-sm-center btn"
              onClick={() => {
          ...
          <nav
            className={`navbar-nav text-dark col-xs-12 col-sm-4 border-0 pr-0 d-none d-sm-flex ${headerStyles.cart} ${headerStyles.disableSelect}`}
          >
            <div
              onClick={() => {
                handleCartClick();
              }}
              className=" col-sm-6 p-0 text-center row m-0"
            >
              <div className="my-auto p-0 mx-auto d-inline-flex justify-content-center">
                <i className="material-icons  mb-0 mr-2 my-auto">
                  <ShoppingCart />
                </i>

                <p className="align-middle my-auto font-weight-bold  mr-2 my-auto">
            ...
            <div
              ...
              className="col-sm-6 text-right p-0 text-center row m-0"
            >
        ...
        {/* Mobile */}
        <div
          className={`row p-0 overflow-hidden d-flex d-md-none row h-100 mx-auto ${headerStyles.navbarXS}`}
        >
          <div className="col-7 h-100 pr-0">
            <div className="row h-100">
              <div
                className="col-2 m-auto pt-2 pr-0 pl-2"
                onClick={() => {
                  handleMenuClick();
                }}
              >
                <span className="material-icons text-success">menu</span>
              </div>
          ...
          <div className="col-5 ">
            <div className="row h-100">
              ...
              <div
                ...
                onClick={() => {
                  handleSearchboxMobileClick();
                }}
              >
                <i className={`material-icons p-0 float-right`}>search</i>
              </div>

              <div
                className="col-4 m-auto text-left pl-0"
                onClick={() => {
                  handleCartClick();
                }}
              >
                <i className="material-icons float-right my-auto">shopping_cart</i>
      ...
      <div
        className={`w-100 ${headerStyles.searchbarXS} d-${
          props.searchboxMobileActive ? 'block' : 'none'
        } d-md-none`}
      >
        <input
          className="form-control mr-sm-2"
